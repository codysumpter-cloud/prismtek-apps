...
Dim log
log = "C:\Users\cody_\prismtek-push\prismtek-apps\stash-rebase-output.txt"
...
cmd = "powershell.exe -ExecutionPolicy Bypass -NoProfile -Command " & Chr(34) & _
    ...
    "$out += '--- git stash (push all staged/unstaged) ---'; " & _
    "$out += (git stash push --include-untracked -m 'helper-scripts-temp' 2>&1); " & _
    "$out += 'stash exit: ' + $LASTEXITCODE; " & _
...
Set f = objFSO.CreateTextFile("C:\Users\cody_\prismtek-push\prismtek-apps\stash-rebase-done.txt", True)
