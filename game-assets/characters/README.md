...
| Buddy detail pack | `game-assets/characters/Buddy (2).zip` | Main Buddy source/reference pack | This is the primary Buddy pack. Use it as the source of truth for Buddy's readable face, proportions, charm, and identity before deriving smaller runtime variants. Keep this as a protected source pack, not a scratch/export folder. |
| Buddy Grok 64 | `game-assets/characters/Buddy_Grok_64.zip` | 64px Buddy/Grok runtime variant | Use as the compact animation/game-runtime variant when a 64px frame target is needed. It should stay visually aligned to `Buddy (2).zip`, not replace it as the main Buddy reference. |
| Buddy Grok Showcase | `game-assets/characters/Buddy_Grok_Showcase.zip` | Showcase/reference bundle for Buddy Grok 64 | Use for previews, readme art, comparisons, and visual QA receipts. Do not treat showcase material as the source of truth if `Buddy (2).zip` is present. |
| Buddy no-heart variant | `game-assets/characters/Buddy_no_heart.zip` | Buddy alternate reference | Use where the Buddy silhouette is wanted without the heart/chest-emblem detail. Treat it as a variant of the `Buddy (2).zip` source pack. |
...
- Keep `Buddy (2).zip` mapped as the main Buddy source/reference pack unless it is intentionally renamed in a dedicated asset-cleanup PR.
...
Use `Buddy (2).zip` as the main Buddy source/reference pack, `Buddy_Grok_64.zip` as the compact Buddy animation/runtime variant, `Buddy_Grok_Showcase.zip` for previews/showcase, `Normal_Hair_Guy.zip` as the active male base, and `Female_Character_Blue_Hoodie.zip` as the active female base.
