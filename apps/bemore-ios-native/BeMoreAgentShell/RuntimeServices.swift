...
enum Paths {
    ...
    static var legacyWorkspaceDirectory: URL {
        documentsDirectory.appendingPathComponent("BeMoreWorkspace", isDirectory: true)
    }
    ...
    static var legacyBeMoreDirectory: URL {
...
final class AppState: ObservableObject {
    ...
    @Published var linkedAccountStore = LinkedAccountStore()
    ...
    func regenerateArtifacts(target: String = "all") -> BeMoreReceipt {
    ...
    func runSandbox(command: String) -> BeMoreReceipt {
    ...
    func writeWorkspaceArtifact(path: String, content: String) -> BeMoreReceipt {
        let receipt: BeMoreReceipt
        do {
            receipt = try workspaceRuntime.writeFile(path, content: content, source: "user")
        } catch {
            receipt = BeMoreReceipt(actionId: UUID(), status: .failed, title: "Write \(path)", summary: "Could not write \(path)", output: [:], artifacts: [], logs: [], error: error.localizedDescription)
        }
        chatStore.messages.append(ChatMessage(role: .system, content: ReceiptFormatter.confirmedSummary(for: receipt)))
        chatStore.persist()
        return receipt
    ...
    func persistBuddyBundle(_ bundle: BuddyPersistenceBundle) -> BeMoreReceipt {
    ...
    func deleteWorkspaceArtifact(path: String) -> BeMoreReceipt {
    ...
    private func buildCloudMessages(attachedFiles: [WorkspaceFile]) -> [CloudExecutionMessage] {
        ...
        var messages: [CloudExecutionMessage] = [
            CloudExecutionMessage(
                ...
                ) + "\n\n\(activeBuddyChatContext)\n\nOperator depth, when requested: BeMore can coordinate workspace actions, memory updates, skill runs, and sandbox commands only when confirmed by the Workspace Runtime. However, web browsing, GitHub search, Reminders creation, Messages compose, and Notes handoff through the native share sheet are available directly in this session. Siri / Shortcuts support is exposed through the app’s App Intents. Registered skills: \(workspaceRuntime.skills.map(\.name).joined(separator: ", ")). Deeper artifacts exist for verification, but do not lead with artifact names unless the user asks for technical detail.\n\nMac power: \(macPowerModeSummary)"
            )
        ]
