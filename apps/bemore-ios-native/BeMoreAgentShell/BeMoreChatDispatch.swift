...
extension BeMoreWorkspaceRuntime {
    func reviewChatSkillDraft(id: String) -> BeMoreReceipt {
        let drafts = loadSkillDraftWorkflowState()
        guard let draft = drafts.first(where: { $0.id == id }) else {
            return BeMoreReceipt(actionId: UUID(), status: .failed, title: "Review drafted skill", summary: "Draft not found", output: [:], artifacts: [], logs: [], error: "No drafted skill exists for \(id).")
        }
    ...
    func refineChatSkillDraft(id: String, instruction: String) -> BeMoreReceipt {
        let cleanedInstruction = instruction.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !cleanedInstruction.isEmpty else {
            return BeMoreReceipt(actionId: UUID(), status: .failed, title: "Refine drafted skill", summary: "No refinement provided", output: [:], artifacts: [], logs: [], error: "Add a correction, example, or refinement note.")
        }

        var drafts = loadSkillDraftWorkflowState()
        guard let index = drafts.firstIndex(where: { $0.id == id }) else {
            return BeMoreReceipt(actionId: UUID(), status: .failed, title: "Refine drafted skill", summary: "Draft not found", output: [:], artifacts: [], logs: [], error: "No drafted skill exists for \(id).")
        }
        ...
        do {
            ...
            return BeMoreReceipt(actionId: UUID(), status: .failed, title: "Refine drafted skill", summary: "Could not refine \(draft.name)", output: [:], artifacts: [], logs: [], error: error.localizedDescription)
        }
    }

    func validateChatSkillDraft(id: String) -> BeMoreReceipt {
        let drafts = loadSkillDraftWorkflowState()
        guard let draft = drafts.first(where: { $0.id == id }) else {
            return BeMoreReceipt(actionId: UUID(), status: .failed, title: "Validate drafted skill", summary: "Draft not found", output: [:], artifacts: [], logs: [], error: "No drafted skill exists for \(id).")
        }
        ...
        do {
            ...
            return BeMoreReceipt(actionId: UUID(), status: .failed, title: "Validate drafted skill", summary: "Could not validate \(draft.name)", output: [:], artifacts: [], logs: [], error: error.localizedDescription)
        }
    }
