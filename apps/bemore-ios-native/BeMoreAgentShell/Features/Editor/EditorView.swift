...
struct EditorTabView: View {
    ...
    private var heroCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Spacer()
                StatusBadge(label: "Native pixel tools", color: BMOTheme.success)
            }

            Text("Use Pixel to sketch, ink, mirror, duplicate frames, and build a small animation loop without leaving the app.")
                .font(.caption)
                .foregroundColor(BMOTheme.textSecondary)
        }
        .bmoCard()
    ...
    private var buddyAssistCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("These buttons use the same local Buddy artifact flow that chat uses for pixel-art help.")
                .font(.caption)
                .foregroundColor(BMOTheme.textSecondary)
