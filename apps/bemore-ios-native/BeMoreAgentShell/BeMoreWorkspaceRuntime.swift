...
enum BeMoreActionStatus: String, Codable, CaseIterable, Hashable {
    case planned
    case queued
    case running
    case completed
...
enum BeMoreActionKind: String, Codable, CaseIterable, Hashable {
...
struct BeMoreActionRecord: Identifiable, Codable, Hashable {
    var id: UUID
    var kind: BeMoreActionKind
    var source: String
    var title: String
    var status: BeMoreActionStatus
    var createdAt: Date
    var updatedAt: Date
...
struct BeMoreReceipt: Identifiable, Codable, Hashable {
...
enum ReceiptFormatter {
    static func confirmedSummary(for receipt: BeMoreReceipt) -> String {
...
struct BeMoreArtifactMetadata: Identifiable, Codable, Hashable {
...
struct BeMoreEventRecord: Codable, Hashable {
    var id: UUID = UUID()
    var type: String
    var message: String
...
final class BeMoreWorkspaceRuntime: ObservableObject {
    ...
    @Published private(set) var artifacts: [BeMoreArtifactMetadata] = []
    @Published private(set) var recentActions: [BeMoreActionRecord] = []
    @Published private(set) var recentEvents: [BeMoreEventRecord] = []
    ...
    private func migrateLegacyRuntimeIfNeeded() {
        let legacy = Paths.legacyBeMoreDirectory
    ...
    func writeFile(_ path: String, content: String, source: String = "runtime") throws -> BeMoreReceipt {
    ...
    func deleteFile(_ path: String, source: String = "runtime") -> BeMoreReceipt {
    ...
    func persistBuddyBundle(_ bundle: BuddyPersistenceBundle, source: String = "buddy.runtime") -> BeMoreReceipt {
    ...
    func regenerateArtifacts(target: String = "all", config: StackConfig, preferences: UserPreferences, routeSummary: String, source: String = "runtime") -> BeMoreReceipt {
        ...
        do {
            ...
            refreshMetadata()
            let status: BeMoreActionStatus = written.isEmpty ? .failed : .persisted
    ...
    func refreshMemory(config: StackConfig, preferences: UserPreferences, routeSummary: String, source: String = "runtime") -> BeMoreReceipt {
    ...
    func runSkill(id: String, input: [String: String], config: StackConfig, preferences: UserPreferences, routeSummary: String) -> BeMoreReceipt {
        ...
        case "builtin.githubSearch":
            ...
            return BeMoreReceipt(actionId: action.id, status: .running, title: "Searching GitHub...", summary: "Searching for \(query)...", output: [:], artifacts: [], logs: [], error: nil)
    ...
    func installBuddySkillTemplate(_ template: BuddySkillTemplate) -> BeMoreReceipt {
    ...
    func draftSkillFromChat(request: String, requestedBy: String) -> BeMoreReceipt {
    ...
    func approveChatSkillDraft(id: String) -> BeMoreReceipt {
    ...
    func runSandbox(command: String, config: StackConfig, preferences: UserPreferences, routeSummary: String) -> BeMoreReceipt {
    ...
    func registerSkill(manifest: SkillManifest, instructions: String? = nil, source: String = "runtime") -> BeMoreReceipt {
    ...
    private func runPokemonTeamBuilder(input: [String: String], manifest: SkillManifest) -> BeMoreReceipt {
    ...
    private func runMemoryInspector(manifest: SkillManifest) -> BeMoreReceipt {
    ...
    private func runManifestSkillWorkflow(manifest: SkillManifest, input: [String: String]) -> BeMoreReceipt {
        let action = begin(kind: .skillRun, source: "skill.\(manifest.id)", title: manifest.name, input: input)
    ...
    private func begin(kind: BeMoreActionKind, source: String, title: String, input: [String: String]) -> BeMoreActionRecord {
        let action = BeMoreActionRecord(id: UUID(), kind: kind, source: source, title: title, status: .queued, createdAt: .now, updatedAt: .now, input: input, output: [:], error: nil, artifacts: [], logs: [])
    ...
    private func finish(_ action: BeMoreActionRecord, status: BeMoreActionStatus, summary: String, output: [String: String] = [:], artifacts: [String] = [], logs: [String] = [], error: String? = nil) -> BeMoreReceipt {
        var finished = action
        ...
        refreshMetadata()
        return BeMoreReceipt(actionId: action.id, status: status, title: action.title, summary: summary, output: output, artifacts: artifacts, logs: logs, error: error)
    }

    private func upsertRecent(_ action: BeMoreActionRecord) {
        recentActions.removeAll { $0.id == action.id }
        recentActions.insert(action, at: 0)
        recentActions = Array(recentActions.prefix(30))
    }

    private func persistAction(_ action: BeMoreActionRecord) {
    ...
    private func appendEvent(type: String, message: String, metadata: [String: String] = [:]) {
        let event = BeMoreEventRecord(type: type, message: message, metadata: metadata)
    ...
    private func loadRecentActions() -> [BeMoreActionRecord] {
        ...
        var latest: [UUID: BeMoreActionRecord] = [:]
        for line in text.split(separator: "\n").suffix(120) {
            if let data = String(line).data(using: .utf8), let action = try? decoder.decode(BeMoreActionRecord.self, from: data) {
    ...
    private func loadRecentEvents() -> [BeMoreEventRecord] {
        ...
        return text.split(separator: "\n").suffix(30).compactMap { line in
            ...
            return try? decoder.decode(BeMoreEventRecord.self, from: data)
...
struct BuddyRuntimeStatus {
    ...
    var artifacts: [BeMoreArtifactMetadata]
    var installedBuddyCount: Int
    var hasActiveBuddy: Bool
    var registeredSkillCount: Int
    var recentChanges: [BeMoreEventRecord]
    var failedActions: [BeMoreActionRecord]
