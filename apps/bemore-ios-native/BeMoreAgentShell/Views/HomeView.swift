    ...
    private var runtimeStatusCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if appState.usesStubRuntime {
                Text("Local model routing is only fully live when the on-device runtime package is present. This build now says that plainly: use a cloud route today, or keep a local model selected as your preferred target when the runtime is available.")
                    .font(.caption)
