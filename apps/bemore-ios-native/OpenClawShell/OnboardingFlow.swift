...
private enum GuidedBuddyAppearanceMode: String, CaseIterable, Identifiable {
    case ascii
    case pixel
...
struct OnboardingFlow: View {
    ...
    @State private var appearanceMode: GuidedBuddyAppearanceMode = .ascii
    @State private var guidedPaletteID = "mint_cream"
    @State private var guidedASCIIVariantID = "starter_a"
    @State private var guidedPixelBodyStyle = "compact"
    @State private var guidedAccessory = "signal scarf"
    @State private var guidedEnergyTone = "friendly"
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            guidedPaletteID = buddyStore.contracts?.creationOptions.defaults.palette ?? guidedPaletteID
            guidedASCIIVariantID = buddyStore.contracts?.creationOptions.defaults.asciiVariant ?? guidedASCIIVariantID
            guidedPixelBodyStyle = buddyStore.contracts?.creationOptions.defaults.bodyStyle ?? guidedPixelBodyStyle
        }
    }
        ...
        ScrollView {
            VStack(spacing: BMOTheme.spacingLG) {
                ...
                VStack(spacing: 12) {
                    ...
                    summaryRow(icon: "paintbrush.pointed", label: "Look", value: guidedAppearanceSummary)
    ...
    private func runBuildSequence() {
        ...
        let steps = [
            ...
            (0.50, appearanceMode == .pixel ? "Seeding a guided pixel-appearance project for \(name)" : "Saving Hermes-native ASCII look for \(name)"),
    ...
    private func finishOnboarding() {
        ...
        appState.buddyStore.personalizeActive(
            displayName: cleanedBuddyName,
            nickname: nil,
            currentFocus: cleanedFocus,
            palette: guidedPaletteID,
            asciiVariantID: guidedASCIIVariantID,
            using: appState
        )
        appState.buddyStore.saveAppearanceProfile(
            profileName: appearanceMode == .pixel ? "Pixel Guided Look" : "Hermes Guided Look",
            archetype: selectedTemplate.map { CouncilBuddyIdentityCatalog.identity(for: $0).archetype } ?? "console_pet",
            palette: guidedPaletteID,
            asciiVariantID: guidedASCIIVariantID,
            pixelVariantID: appearanceMode == .pixel ? guidedPixelBodyStyle : nil,
            expressionTone: guidedEnergyTone,
            accentLabel: appearanceMode == .pixel ? "\(guidedPixelBodyStyle) • \(fallback(guidedAccessory, defaultValue: "starter charm"))" : fallback(guidedAccessory, defaultValue: "starter glow"),
            setActive: true,
            using: appState
        )
        if appearanceMode == .pixel {
            PixelStudioStore.shared.update {
                $0.title = "\(cleanedBuddyName) Sprite"
                $0.author = appState.operatorDisplayName
                $0.concept = "A guided pixel Buddy based on \(selectedTemplate?.name ?? "Buddy") with a \(guidedPixelBodyStyle) silhouette and \(fallback(guidedAccessory, defaultValue: "starter charm"))."
                $0.canvasSize = 32
                $0.frameCount = 4
                $0.palette = (paletteOption(for: guidedPaletteID)?.colors.joined(separator: ", ")) ?? $0.palette
                $0.polishGoal = "Keep the silhouette readable and make the \(fallback(guidedAccessory, defaultValue: "starter accent")) stand out on mobile."
                $0.animationGoal = "Build a short idle loop that feels \(guidedEnergyTone)."
                $0.notes = "Seeded from onboarding with guided pixel Buddy answers."
            }
        }
    ...
    private var selectedBuddyPreview: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(selectedTemplate?.ascii.baseSilhouette ?? "    /\\\n  < o  o >\n  /|  v |\\\n /_|____|_\\")
                ...
                .foregroundColor(BMOTheme.accent)
            Text("\(fallback(buddyName, defaultValue: selectedTemplate?.name ?? "Buddy")) will become the Buddy shown on Home and in Chat. Guided look: \(guidedAppearanceSummary).")
                .font(.subheadline)
                .foregroundColor(BMOTheme.textSecondary)
        }
        .bmoCard()
    ...
    private var guidedAppearanceCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Guided Appearance")
                        .font(.headline)
                        .foregroundColor(BMOTheme.textPrimary)
                    Text("Use the same guided creation idea for both Buddy render paths. ASCII stays the built-in default. Pixel mode seeds Studio with a consistent body, palette, accessory, and energy vibe.")
                        .font(.caption)
                        .foregroundColor(BMOTheme.textSecondary)
                }
                Spacer()
                StatusBadge(label: appearanceMode.title, color: appearanceMode == .pixel ? BMOTheme.accent : BMOTheme.success)
            }

            Picker("Render mode", selection: $appearanceMode) {
                ForEach(GuidedBuddyAppearanceMode.allCases) { mode in
                    Text(mode.title).tag(mode)
                }
            }
            .pickerStyle(.segmented)

            Picker("Palette", selection: $guidedPaletteID) {
                ForEach(availablePaletteOptions, id: \.id) { palette in
                    Text(palette.label).tag(palette.id)
                }
            }
            .pickerStyle(.menu)

            if appearanceMode == .ascii {
                Picker("ASCII style", selection: $guidedASCIIVariantID) {
                    ForEach(asciiVariantOptions, id: \.self) { option in
                        Text(option.capitalized).tag(option)
                    }
                }
                .pickerStyle(.segmented)
            } else {
                Picker("Pixel body style", selection: $guidedPixelBodyStyle) {
                    ForEach(pixelBodyStyleOptions, id: \.self) { option in
                        Text(option.capitalized).tag(option)
                    }
                }
                .pickerStyle(.segmented)
            ...
            TextField(appearanceMode == .pixel ? "Accessory or prop" : "Accent or charm", text: $guidedAccessory)
            ...
            Picker("Energy", selection: $guidedEnergyTone) {
                ForEach(energyToneOptions, id: \.self) { option in
                    Text(option.capitalized).tag(option)
                }
            }
            .pickerStyle(.segmented)
    ...
    private var generatedChecklist: [String] {
        ...
        items.append("Guided look selected: \(guidedAppearanceSummary).")
    ...
    private var availablePaletteOptions: [BuddyPaletteOption] {
        buddyStore.contracts?.creationOptions.options.palettes ?? [BuddyPaletteOption(id: guidedPaletteID, label: guidedPaletteID, colors: ["#9EF0D0", "#2B7A78", "#17252A", "#FEF6E8"])]
    }

    private var pixelBodyStyleOptions: [String] {
        (buddyStore.contracts?.creationOptions.options.bodyStyles.map(\.id) ?? ["compact", "round", "floaty", "angular"]).prefix(4).map { $0 }
    }

    private var asciiVariantOptions: [String] {
        ["starter_a", "starter_b", "starter_c"]
    }

    private var energyToneOptions: [String] {
        ["friendly", "curious", "focused"]
    }

    private var guidedAppearanceSummary: String {
        if appearanceMode == .pixel {
            return "Pixel • \(guidedPixelBodyStyle) • \(paletteOption(for: guidedPaletteID)?.label ?? guidedPaletteID) • \(fallback(guidedAccessory, defaultValue: "starter charm")) • \(guidedEnergyTone)"
        }
        return "ASCII • \(guidedASCIIVariantID) • \(paletteOption(for: guidedPaletteID)?.label ?? guidedPaletteID) • \(guidedEnergyTone)"
    }

    private func paletteOption(for id: String) -> BuddyPaletteOption? {
        availablePaletteOptions.first(where: { $0.id == id })
    }
