...
4. There is no structured runtime diagnostics path yet that clearly distinguishes:
   - missing model asset
   - bad model/runtime binding
   - empty generation output
   - memory pressure or forced unload
5. Because those pieces are missing, any PR that claims the local runtime is complete would be dishonest.

## Execution map

Use `apps/bemore-ios-native/LOCAL_RUNTIME_IMPLEMENTATION_PLAN.md` as the concrete execution plan for resolving these blockers. That file points at the exact source files and the recommended order of work.
...
- runtime failures are observable and classified instead of silently failing
