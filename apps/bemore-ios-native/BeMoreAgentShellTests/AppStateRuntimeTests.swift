...
final class AppStateRuntimeTests: XCTestCase {
    ...
    func testWorkspaceBootstrapCreatesCanonicalBeMoreArtifacts() throws {
        ...
        for path in ["soul.md", "user.md", "memory.md", "session.md", "skills.md", "registry/skills.json", "state/facts.json", "state/preferences.json", "state/tasks.json", "state/session.json"] {
            XCTAssertTrue(FileManager.default.fileExists(atPath: Paths.beMoreRuntimeDirectory.appendingPathComponent(path).path), path)
        }
    ...
    func testPokemonTeamBuilderPersistsArtifactsThroughGenericRunner() throws {
        ...
        XCTAssertTrue(receipt.artifacts.allSatisfy { FileManager.default.fileExists(atPath: Paths.beMoreRuntimeDirectory.appendingPathComponent($0).path) })
    ...
    func testBuddySkillTemplateInstallsRegistryBackedSkill() throws {
        let runtime = BeMoreWorkspaceRuntime()
        runtime.bootstrap(config: .default, preferences: .default, routeSummary: "Route not configured")

        let template = try XCTUnwrap(BuddySkillCatalog.templates.first)
        XCTAssertGreaterThanOrEqual(BuddySkillCatalog.templates.count, 3)
        let receipt = runtime.installBuddySkillTemplate(template)

        XCTAssertEqual(receipt.status, .persisted)
        XCTAssertTrue(runtime.skills.contains(where: { $0.id == template.id }))
        XCTAssertTrue(FileManager.default.fileExists(atPath: Paths.beMoreRuntimeDirectory.appendingPathComponent("skills/\(template.id)/README.md").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: Paths.beMoreRuntimeDirectory.appendingPathComponent("skills/\(template.id)/manifest.json").path))
        ...
        XCTAssertEqual(runReceipt.output["entrypoint"], "buddyskill.\(template.id)")
        XCTAssertTrue(runReceipt.artifacts.count == 1)
        XCTAssertTrue(runReceipt.summary.contains("saved a run log"))
    }
