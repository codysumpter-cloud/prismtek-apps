...
final class BeMoreCapabilityMirrorTests: XCTestCase {
    ...
    func testCapabilityMirrorMarksNativeAndLinkedCapabilitiesHonestly() {
        ...
        XCTAssertTrue(statuses.contains(where: { $0.capability.id == "admin.builder" && $0.availability == .available }))
        XCTAssertTrue(statuses.contains(where: { $0.capability.id == "admin.mission-control" && $0.availability == .available }))
        XCTAssertTrue(statuses.contains(where: { $0.capability.id == "profile.manage" && $0.availability == .available }))
