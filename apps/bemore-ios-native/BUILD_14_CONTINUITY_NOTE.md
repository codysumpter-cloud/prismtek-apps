...
The correct continuity strategy for this TestFlight line is to keep `PRODUCT_BUNDLE_IDENTIFIER: BeMoreAgent`. Current `main` restores that value and the TestFlight workflow now guards it. Users upgrading from builds that used `BeMoreAgent` should keep their existing local app-container state.
