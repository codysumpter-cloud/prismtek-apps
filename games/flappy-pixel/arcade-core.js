...
export function createArcadeGame(config) {
  ...
  const bestKey = `prismtek.arcade.${config.slug}.best`;
  const best = Number(localStorage.getItem(bestKey) ?? 0);
  root.innerHTML = `
    <section class="shell">
      <header>
        <div><p class="eyebrow">Prismtek Arcade</p><h1>${config.title}</h1><p>${config.subtitle ?? "Low-resource arcade run."}</p></div>
        <div class="score"><span id="score">0</span><small>score</small><span id="best">PB ${best}</span></div>
      </header>
      <canvas id="game" width="480" height="320" tabindex="0" aria-label="${config.title} canvas"></canvas>
      <footer><span id="status">${config.instructions}</span><button id="restart">Restart</button></footer>
      <pre id="receipt" class="receipt" aria-live="polite"></pre>
    </section>`;
  ...
  const receiptEl = root.querySelector("#receipt");
  ...
  function frame(now) {
    ...
    const bestScore = Math.max(best, Math.floor(state.score));
    bestEl.textContent = `PB ${bestScore}`;
    if (state.gameOver && !receiptWritten) {
      receiptWritten = true;
      ...
      const receipt = {
        ...
        best: bestScore,
        durationMs: Math.floor(state.t * 1000),
        endedAt: new Date().toISOString(),
        rank: rankFor(state.score)
      };
      receiptEl.textContent = JSON.stringify(receipt, null, 2);
    }
    statusEl.textContent = state.gameOver ? `${config.title} ended — press Restart for the rematch` : config.instructions;
    input.pressed.clear();
    input.pointer = false;
...
function update(slug, s, dt, keys, input) {
  ...
  if (slug === "pixel-snake") updateSnake(s, dt, keys);
...
function updateFlappy(s, dt, input) {
  if (input.pressed.has(" ") || input.pointer) s.bird.vy = -255;
...
function updateSnake(s, dt, keys) {
...
function updateStacker(s, dt, input) {
  ...
  if (!input.pressed.has(" ") && !input.pointer) return;
...
function rankFor(score) { if (score >= 500) return "Prism Legend"; if (score >= 300) return "Neon Myth"; if (score >= 180) return "Circuit King"; if (score >= 90) return "Rail Shark"; return "Alley Spark"; }
