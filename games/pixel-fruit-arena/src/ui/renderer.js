...
export class Renderer {
  ...
  draw(snapshot, mode, options = {}) {
    ...
    if (stage.theme === "boxing") drawBoxingBackdrop(ctx, snapshot.elapsed, stage);
      ...
      drawRuinsBackdrop(ctx, this.images.get(STAGE_ART.skyRuins.tileset));
    }
    for (const p of stage.platforms) stage.theme === "boxing" ? drawBoxingPlatform(ctx, p) : drawPlatform(ctx, p, this.images.get(STAGE_ART.skyRuins.tileset));
...
function drawRuinsBackdrop(ctx, tileset) {
  ...
  if (isReady(tileset)) {
    STAGE_ART.skyRuins.fruitTiles.forEach((tile, index) => blit(ctx, tileset, tile, 338 + index * 96, 178 + (index % 2) * 24, 32, 32));
  }
}
...
function drawBoxingPlatform(ctx, p) {
  ctx.fillStyle = p.type === "solid" ? "#e8e0cf" : "#27405c";
  ctx.fillRect(p.x, p.y, p.w, p.h);
  ctx.fillStyle = p.type === "solid" ? "#cfc4ab" : "#5d7190";
...
function drawRingRopes(ctx, stage) {
  ...
  if (!ring) return;
  const colors = ["#ef476f", "#f8fafc", "#118ab2"];
  for (const post of [ring.postLeft, ring.postRight]) {
    ctx.fillStyle = "#31415d";
    ctx.fillRect(post.x, post.y, 16, ring.floor.y - post.y + 8);
  }
  ring.ropeYs.forEach((y, i) => {
    ctx.save();
    ctx.globalAlpha = 0.5;
    ctx.strokeStyle = colors[i % colors.length];
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(ring.postLeft.x + 8, y);
    ctx.lineTo(ring.postRight.x + 8, y);
    ctx.stroke();
    ctx.restore();
  });
}
...
function drawFighter(ctx, f, images) {
  ...
  const sprite = CHARACTER_SPRITES[f.spriteKey] || CHARACTER_SPRITES.male_basic || CHARACTER_SPRITES.pink;
...
function drawCustomLayers(ctx, f, sprite, dx, dy) {
  ...
  // Clothing body variants, layered over the base sheet so all bodies can share the same customization system.
  ...
  // Hair styles from the imported hairstyle/clothes concept, rendered as pixel overlays.
...
function drawEffect(ctx, effect, referenceManifest, images) {
  ...
  if (effect.type === "attack") {
    if (!drawVfxSheet(ctx, effect, images) && !drawReferenceEffect(ctx, effect, referenceManifest, images, alpha)) {
