...
export function renderHud(node, snapshot, fruits) {
  node.innerHTML = snapshot.fighters.map((f) => {
    ...
    return `<article class="hud-card ${f.awakened > 0 ? "is-awakened" : ""} ${f.hakiGuard > 0 ? "is-haki" : ""}" style="--fruit:${fruit.color};--health:${health}%;--haki:${haki}%">
      ...
      <div class="hud-bars" aria-label="health, awakening, and haki meters"><meter class="health-meter" min="0" max="100" value="${health}"></meter><meter class="awakening-meter" min="0" max="100" value="${f.awakening}"></meter><meter class="haki-meter" min="0" max="100" value="${haki}"></meter></div>
...
function mainMenu(props) {
  return `<header class="menu-header"><h1>Pixel Fruit Arena</h1><p>Local platform fighting prototype</p></header>
    <nav class="menu-actions main-nav"><button class="primary" data-open-lobby="2" data-cpu="true">Fight CPU</button><button data-open-lobby="1" data-training="true">Training Room</button><button data-open-lobby="2">2 Players</button><button data-open-lobby="3">3 Players</button><button data-open-lobby="4">4 Players</button></nav>
...
function creatorScreen(props) {
  return `<header class="menu-header compact"><h2>Character Creator</h2><p>Pick a playable body, then customize hair, clothing, colors, and fight style.</p></header>
    <section class="creator">
      ...
      ${appearanceChoice("Clothing", "clothingStyle", props.cosmetics.clothingStyles, props.profile.appearance.clothingStyle)}
...
function fruitsScreen(props) {
  return `<header class="menu-header compact"><h2>Fruit Powers</h2><p>Every fruit has three base moves, directional variants, and stronger awakened versions.</p></header>
    <section class="fruit-grid">${Object.values(props.fruits).map((fruit) => `<article class="fruit-card ${props.profile.equipped_fruit === fruit.id ? "is-equipped" : ""}" style="--fruit:${fruit.color}"><h3>${fruitIcon(fruit, 22)} ${fruit.name}</h3><ol class="ability-list">${fruit.abilities.map((a, i) => `<li><b>${i + 1}</b> ${escapeHtml(a.name)} <small>${escapeHtml(titleCase(a.kind))}</small></li>`).join("")}</ol><strong>Awakening: ${escapeHtml(fruit.awakening)}</strong><small>${escapeHtml(fruit.awakeningEffect || "Awakening upgrades damage, range, cooldown, and hit pressure.")}</small><small>Mastery ${Math.round(masteryFor(props.profile, fruit.id))}</small><button data-equip="${fruit.id}">${props.profile.equipped_fruit === fruit.id ? "Equipped" : "Equip"}</button></article>`).join("")}</section>
    <nav class="menu-actions"><button data-mode="menu" data-back>Back</button></nav>`;
}

function controlsScreen() {
  return `<header class="menu-header compact"><h2>Controls</h2><p>Attacks 1, 2, 3 are your equipped fruit's three abilities.</p></header><section class="controls-grid">
...
function lobbyScreen(props) {
  ...
  return `<header class="menu-header compact"><h2>${title}</h2><p>Set up every fighter, then start the match.</p></header><section class="stage-row"><span class="row-label">Stage</span>${Object.values(props.stages).map((stage) => `<button class="choice-button ${lobby.stageId === stage.id ? "is-selected" : ""}" data-stage="${stage.id}" type="button">${escapeHtml(stage.name)}</button>`).join("")}</section><section class="lobby-grid players-${lobby.playerCount}">${lobby.players.slice(0, lobby.playerCount).map((player, slot) => playerCard(player, slot, props)).join("")}</section><nav class="menu-actions"><button class="primary" data-begin-fight>Start ${lobby.training ? "Training" : "Fight"}</button><button data-mode="menu" data-back>Back</button></nav><footer>Tip: Haki can guard or empower pressure. ${deviceSummary(props)}</footer>`;
}
...
function playerCard(player, slot, props) {
  ...
  return `<article class="player-card" data-slot="${slot}" style="--fruit:${fruit.color}"><header><b>P${slot + 1}</b><input data-lobby-name="${slot}" value="${escapeHtml(player.name)}" maxlength="18" aria-label="Player ${slot + 1} name" ${isCpu ? "disabled" : ""}></header><label class="device-row">Device<select data-lobby-device="${slot}">${deviceOptions.map((id) => `<option value="${id}" ${player.device === id ? "selected" : ""}>${DEVICE_LABELS[id] || id}</option>`).join("")}</select></label>${lobbyChoice(slot, "Body", "sprite_key", props.cosmetics.spriteKeys, player.sprite_key, BODY_NAMES)}${lobbyChoice(slot, "Hair", "hairStyle", props.cosmetics.hairStyles, player.appearance.hairStyle)}${lobbyChoice(slot, "Clothing", "clothingStyle", props.cosmetics.clothingStyles, player.appearance.clothingStyle)}${lobbySwatches(slot, "Hair Color", "hairColor", props.cosmetics.hairColors, player.appearance.hairColor)}${lobbySwatches(slot, "Skin", "skinTone", props.cosmetics.skinTones, player.appearance.skinTone)}${lobbySwatches(slot, "Outfit", "outfitPrimary", props.cosmetics.outfitColors, player.appearance.outfitPrimary)}${lobbySwatches(slot, "Trim", "outfitSecondary", props.cosmetics.accessoryColors, player.appearance.outfitSecondary)}${lobbySwatches(slot, "Accessory", "accessoryColor", props.cosmetics.accessoryColors, player.appearance.accessoryColor)}${lobbyChoice(slot, "Style", "combat_style", Object.keys(props.combatStyles), player.combat_style, Object.fromEntries(Object.values(props.combatStyles).map((s) => [s.id, s.name])))}<fieldset class="choice-grid fruit-row"><legend>Fruit Power</legend>${Object.values(props.fruits).map((f) => `<button class="choice-button fruit-choice ${player.fruitId === f.id ? "is-selected" : ""}" style="--fruit:${f.color}" data-lobby-fruit="${slot}" data-value="${f.id}" type="button" title="${escapeHtml(f.name)}">${fruitIcon(f, 20)}<span>${escapeHtml(f.name.replace(" Fruit", ""))}</span></button>`).join("")}</fieldset></article>`;
}
...
function appearanceChoice(label, key, values, current) { return `<fieldset class="choice-grid"><legend>${label}</legend>${values.map((value) => `<button class="choice-button ${value === current ? "is-selected" : ""}" data-appearance="${key}" data-value="${value}" type="button">${escapeHtml(titleCase(value))}</button>`).join("")}</fieldset>`; }
...
function lobbyChoice(slot, label, key, values, current, names = {}) { return `<fieldset class="choice-grid"><legend>${label}</legend>${values.map((value) => `<button class="choice-button ${value === current ? "is-selected" : ""}" data-lobby-option="${slot}" data-key="${key}" data-value="${value}" type="button">${escapeHtml(names[value] || titleCase(value))}</button>`).join("")}</fieldset>`; }
