...
export const PRISMTEK_FRUIT_ENCYCLOPEDIA=ROWS.split(";").filter(Boolean).map(r=>{const [id,baseName,rarity,fruitClass,powerKey]=r.split(",");const kinds=KINDS[fruitClass]||KINDS.utility;const b=BONUS[rarity]??0;return {id,name:`${baseName} Fruit`,rarity,class:fruitClass,powerKey,color:COLORS[fruitClass]||COLORS.utility,icon:baseName[0],awakening:`${baseName} Overdrive`,awakeningEffect:"Temporarily intensifies movement, hitboxes, and arena-control.",abilities:MOVES.map((m,i)=>{const kind=kinds[i%kinds.length];return {id:`${id}_${i+1}`,name:`${baseName} ${m}`,kind,damage:8+i*3+b,knockback:310+i*120+b*20,cooldown:Number((0.52+i*.27+b*.025).toFixed(2)),speed:["projectile","beam","dash","blink"].includes(kind)?520+i*90:undefined,range:["melee","heavy"].includes(kind)?90+i*20:undefined,vfx:kind};})};});
